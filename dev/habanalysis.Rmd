...
#for(w in wmin:wmax) { # all data
   ...
   title(paste0("Week ",w," (",format(week.as.date(w),format="%d-%b-%Y"),")"))
}
```



Identify dates on which a monitored polygon switches on
```{r}
nrow(event)
#View(event)
```
...
draw.state <- function(yvec, mpoly, 
  ...
  plot((mpoly %>% mutate(plotvalue=yvec))["plotvalue"], reset=FALSE, 
...
ymat[1,] <- yvec
for(i in 2:nstep) {
  ymat[i,] <- beta/vol15vec * (resvec/p0)^lambda2 * pmat%*%((resvec/p0)^lambda1*vol15vec*ymat[i-1,])
}
draw.state(yvec,mpoly,breaks=seq(from=0, to=1, length=11))
```
...
```{r}
draw.state(mpoly$residency, mpoly, zlim=c(0,1), main="Residency", xlim=xlim.nydia, ylim=ylim.nydia)
draw.state(mpoly$residency^lambda1, mpoly, zlim=c(0,1), main=expression("Residency"^(lambda[1])), xlim=xlim.nydia, ylim=ylim.nydia)
draw.state(mpoly$residency^lambda2, mpoly, zlim=c(0,1), main=expression("Residency"^(lambda[2])), xlim=xlim.nydia, ylim=ylim.nydia)
rr <- range(log(1+ymat))
breaks <- seq(from=rr[1], to=rr[2], length=11)
for(i in 1:nstep) {
  draw.state(log(1+ymat[i,]), mpoly, main=i, breaks=breaks, xlim=xlim.nydia, ylim=ylim.nydia)
}
```
