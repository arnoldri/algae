...
#range(marlpoly_sf.latlon$CLUSTER_ID); nrow(marlpoly_sf.latlon)
...
#range(marlpoly_sf.xy$CLUSTER_ID); nrow(marlpoly_sf.xy)
...
```{r}
nw <- nrow(np)
np$onset <- c(np$np[1]>0, np$np[-1]>0 & np$np[-nw]==0)
plot(np$date, np$np)
sum(np$onset) # 29
abline(v=np$date[np$onset],col="red")
```
...
plot(marlpoly_sf.trim %>% filter(CLUSTER_ID%in%ee$CLUSTER_ID) %>% st_geometry(), add=TRUE, col="red ")
```
