...
simstates <- function(pmat, volvec,
                      ...
                      lambda1=1, lambda2=1, beta=1, p0=0.5) {
   npoly <- nrow(pmat)
   yvec <- rep(0,npoly)
   yvec[ic.nydia] <- 1
   ...
   ymat[1,] <- yvec
   for(i in 2:nstep) {
      ymat[i,] <- beta/volvec * (resvec/p0)^lambda2 * pmat%*%((resvec/p0)^lambda1*volvec*ymat[i-1,])
   }
...
```{r}
ic.nydia <- 32
ic.source <- ic.nydia; a0 <- 1; 
lambda1 <- 1; lambda2 <- 6; beta <- 5; p0 <- 0.5

set.seed(-1)
sstate1 <- simstates(pmat, vol15vec, nstep=10, ic.source=ic.source, a0=a0, 
                     lambda1=lambda1, lambda2=lambda2, beta=beta, p0=p0)
draw.state(sstate1$ymat[1,],mpoly,breaks=seq(from=0, to=1, length=11))
```
...
draw.state(mpoly$residency^sstate$lambda1, mpoly, zlim=c(0,1), main=expression("Residency"^(lambda[1])), 
           xlim=xlim.kp, ylim=ylim.kp)
draw.state(mpoly$residency^sstate$lambda2, mpoly, zlim=c(0,1), main=expression("Residency"^(lambda[2])), 
           xlim=xlim.kp, ylim=ylim.kp)
rr <- range(log(1+sstate$ymat))
breaks <- seq(from=rr[1], to=rr[2], length=10)
for(i in 1:nrow(sstate$ymat)) {
  draw.state(log(1+sstate$ymat[i,]), mpoly, main=i, breaks=breaks, xlim=xlim.kp, ylim=ylim.kp)
}
```
...
idx <- mpoly$monitored
for(i in 1:nrow(sstate$ymat)) {
  ss <- bquote(.(i)*" ("*beta==.(beta)*", "*lambda[1]*"="*.(lambda1)*", "*lambda[2]*"="*.(lambda2)*", "*p[0]*"="*.(p0)*")")
  #ss <- bquote("i")
...
xlim <- xlim.kp
ylim <- ylim.kp
rr <- range(sstate$ymat)
rr <- log(1+rr)
breaks <- seq(from=rr[1], to=rr[2], length=10)
for(i in 1:nstep) { 
   
   m <- merge(mpoly, data.frame(CLUSTER_ID=1:npoly, Value=sstate$ymat[i,]))
   m$Value[!m$monitored] <- NA
   m$logValue <- log(1+m$Value)
   plot(mpoly["logValue"],
          reset=FALSE, main="", breaks=breaks, key.pos=4,
          xlim=xlim, ylim=ylim) 
...
```{r}
plot(mpoly %>% st_geometry(), reset=FALSE)
plot(coast %>% st_geometry(), add=TRUE, col="darkgreen")
```
...
for(i in 1:nrow(ymat)) {
  draw.state(log(1+ymat[i,]), mpoly, main=i, breaks=breaks)
}
```
