...
if(getmatrices) {
  ...
  nmat <- length(conn)/2
  migr <- conn[nmat+1:nmat]
  conn <- conn[1:nmat]
  names(migr) <- names(conn)
  matdates <- as.Date(names(conn),format="%Y%m%d")
  save("conn",file="data/conn2018.Rda")
  save("migr",file="data/migr2018.Rda")
} else {
  load("data/conn2018.Rda") # creates conn
  load("data/migr2018.Rda") # creates migr
  matdates <- as.Date(names(conn),format="%Y%m%d")
}
#save.image()
```
...
sapply(migr,dim)-408 # matrices are 408 x 409
```
...
```{r}
dim(migr[[1]]) # 408 rows x 409 columns

range(apply(migr[[1]],1,sum,na.rm=TRUE)) # row sums 0.2671909 1.8655115
range(apply(migr[[1]],2,sum,na.rm=TRUE)) # col sums 0 1

table(apply(migr[[1]],1,sum,na.rm=TRUE)>0) # 408 nonzero,  0 zero
table(apply(migr[[1]],2,sum,na.rm=TRUE)>0) # 408 nonzero,  1 zero
# migr[i,j] has sum_j migr[i,j] = 1 for all i where particles have been released
# i.e. migr[i,j] = proportion of mass in i that came from source polygon j

# which polygon gains no mass?
i1 <- which.min(apply(migr[[1]],2,sum)) # 195 - which has zero volume!
marlpoly_sf %>% filter(CLUSTER_ID==i1)

#hist(apply(migr[[1]],1,sum,na.rm=TRUE))
hist(apply(migr[[1]],2,sum,na.rm=TRUE),
     xlab="Proportion of mass arriving from within the system",
...
range(migr[[1]][,195]) # 0 0 No mass in polygon 195 arrives from anywhere
range(migr[[1]][195,]) # 0 0.2469852: some mass arrives here?
which(migr[[1]][195,]>0) # from 9 polygons...  6  47  75  86 154 162 190 345 396
```
