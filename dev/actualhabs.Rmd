...
* Onepipi PG121 did not have a longitude/latitude (recorded as 0,0): this is corrected in the file "Raw_MSQP_Locations-Corrected.csv"

 - PG121 - Onepipi - did not have lon/lat: corrected to same lon/lat as PG092 Onepipi
 - G062 - D'Urville - fully outside tiled region - no match: OK leave as non-matching
 ...
 - G096 - Te Mahia - on land - and a long way in: clearly wrong!  On MPI map Te Mahia should be at x=1681390.0056, y=5436803.2623
          ...
          Need to fix these coordinates to x=1681390.0056, y=5436803.2623 lon= lat=
 ...
 - PG111 - Kenepuru Head - inland: should be the same as G111 (1693614 5441549), and not as recorded (1689756 5448481)
           Need to fix these coordinates: match to G111
...
gg <- ra_lonlat_to_xy(habloc$lon, habloc$lat, marlcrs)
gg <- ra_xy_to_lonlat(habloc$x, habloc$y, marlcrs)

gg %>% st_drop_geometry

gg <- st_as_sf(habloc[,c("lon","lat")], coords=c("lon","lat"), crs=globalcrs, agr="constant")
gg1 <- st_transform(gg, crs=marlcrs)

plot(st_geometry(gg)); axis(1); axis(2); box()
plot(st_geometry(gg1)); axis(1); axis(2); box()
...
```
 ...
 - G096 - Te Mahia - on land - and a long way in: clearly wrong!  On MPI map Te Mahia should be at x=1681390.0056, y=5436803.2623
          ...
          in our list though G108 is Port Hardy (which is on D'Urville Island...)
          However MPI list of Port Hardy is G108 too, so the MPI map label is wrong too.
          Need to fix these coordinates to x=1681390.0056, y=5436803.2623
 - G105 - Black Rock - OK - this is at the head of an inlet - OK retain
 - G128 - Cook Strait 1 - too far out: remove
 - PG017 - Anakoha Bay - OK - at the end of an inlet - OK retain: match to G017 (174.095 -41.004 1692086 5460222)
 - PG062 - D'Urville - fully outside tiled region - no match: remove
...
plot(st_geometry(marlpoly_sf.trim %>% filter(CLUSTER_ID%in%outofarea_sf$CLUSTER_ID_closest)), col="yellow", add=TRUE)
plot(st_geometry(outofarea_sf), pch=16, col="red", cex=1, add=TRUE)
axis(1); axis(2); box()
```
