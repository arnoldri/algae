...
```{r}
hist(apply(pmat,2,sum), xlab=expression(W[j]=="Retention "*Sigma[i]*P[ij]),
...
apply(imat,2,sum)
plot((nydia_sf %>% mutate(nstart=apply(imat,2,sum)[nid]))['nstart'], main="Number of initiations")
text(nydia_sf %>% st_centroid(of_largest_polygon=TRUE) %>% st_coordinates(), 
     lab=as.character(nydia_sf$CLUSTER_ID), cex=0.5)
```
