...
evec[1] <- rbinom(1,1,tau0*seasonvec[1])
for(i in 2:nt) {
  evec[i] <- if(evec[i-1]==0) rbinom(1,1,tau0*seasonvec[i]) else rbinom(1,1,tau1*seasonvec[i])
}
```
...
```{r}
plot(week.as.date(tvec), seasonvec, type="s", col="grey", 
     xlab="Date", ylab="Bloom favourability", 
     main="Bloom epochs", axes=FALSE)
bb <- block01(tvec,seasonvec)
...
alpha0 <- 0.8; lambda0 <- 3.0
pavec <- volvec*exp(alpha0 + lambda0*log(resvec))
...
```{r}
par(mfrow=c(1,2))
plot(tvec,(mmat[,2]), xlab="Week", ylab="Mass")
abline(v=tvec[evec==1],col="lightgrey")
points(tvec,mmat[,2])
plot(tvec,log(mmat[,2]), xlab="Week", ylab="log(Mass)")
abline(v=tvec[evec==1],col="lightgrey")
...
par(mfrow=c(1,2))
plot(tvec,(mmat[,ipoly]), xlab="Week", ylab="Mass")
abline(v=tvec[evec==1],col="lightgrey")
points(tvec,mmat[,ipoly])
plot(tvec,log(mmat[,ipoly]), xlab="Week", ylab="log(Mass)")
abline(v=tvec[evec==1],col="lightgrey")
...
```{r}
plot(tvec,rep(NA,length(tvec)), ylim=range(cmat), type=NULL,
     xlab="Week",ylab="Concentration")
invisible(apply(cmat,2,function(cvec) lines(tvec,cvec)))
```
...
if(region=="nydia") {
  ...
  eventdat <- event[event$CLUSTER_ID%in%idx,]
  monitorid <- polyid
  ...
  clusteridname <- "CLUSTER_ID"
  pmat <- t(connsub[[1]])
  colnames(pmat) <- polyid
  rownames(pmat) <- colnames(pmat)
  npoly <- nrow(pmat)
  # check: max(apply(pmat,1,sum))==1
  (max(apply(pmat,1,sum))-1) < 3e-16
  covmat <- covsublist[[1]]
  arealabel <- "Marlborough"
...
sobj <- list(npoly=npoly, polysf=polysf, polyid=polyid, pmat=pmat, covmat=covmat, 
             retvec=retvec, resvec=resvec, volvec=volvec,
             clusteridname=clusteridname, arealabel=arealabel,
             coastsf=coastsf) 

datalist <- list(event=eventdat, monitorid=monitorid)

datalist$week1 <- min(eventdat$week)
datalist$week2 <- max(eventdat$week)
datalist$tvec <- datalist$week1:datalist$week2
datalist$nt <- length(datalist$tvec)
datalist$datevec <- week.as.date(datalist$tvec)
datalist$year1 <-as.numeric(format(datalist$datevec[1],"%Y"))
datalist$year2 <-as.numeric(format(datalist$datevec[datalist$nt],"%Y"))

datalist$polyid <- sort(unique(eventdat$CLUSTER_ID))
datalist$npoly <- length(datalist$polyid)
datalist$poly.idx <- match(datalist$polyid,sobj$polysf$CLUSTER_ID)

cmat <- array(NA,dim=c(datalist$nt,datalist$npoly))
dimnames(cmat) <- list(datalist$tvec, datalist$polyid)
cmat[cbind(match(eventdat$week, datalist$tvec),
           match(eventdat$CLUSTER_ID, colnames(cmat)))] <- eventdat$Value
datalist$cmat <- cmat
```
...
```{r warning=FALSE}
plot.sobj(sobj, datalist, type="polygons")
plot.sobj(sobj, datalist, type="volume")
plot.sobj(sobj, datalist, type="retention")
plot.sobj(sobj, datalist, type="residency")
sfreset()
plot.sobj(sobj, datalist, type="communication")
```

Data

````{r}
plot.data(sobj, datalist, type="observed.concentration.over.time") 
```

````{r}
plot.data(sobj, datalist, type="map.concentration", it1=1, it2=5) 
```
...
make.movie <- TRUE
make.movie <- FALSE
#plot.data(sobj, datalist, type="map.concentration",it1=1,it2=datalist$nt,
...
```{r}
plot.sim(simlist, sobj, tlist, parlist, datalist, type="epoch")
plot.sim(simlist, sobj, tlist, parlist, datalist, type="carrying.density")
plot.sim(simlist, sobj, tlist, parlist, datalist, type="map.carrying.density")
sfreset(); 
plot.sim(simlist, sobj, tlist, parlist, datalist, type="carrying.mass")
plot.sim(simlist, sobj, tlist, parlist, datalist, type="map.carrying.mass")
sfreset(); 
plot.sim(simlist, sobj, tlist, parlist, datalist, type="bloom.initiation.probability")
plot.sim(simlist, sobj, tlist, parlist, datalist, type="map.bloom.initiation.probability")
plot.sim(simlist, sobj, tlist, parlist, datalist, type="map.number.of.initiations"); ##sfreset()
sfreset(); 
plot.sim(simlist, sobj, tlist, parlist, datalist, type="bloom.initiation.times")
plot.sim(simlist, sobj, tlist, parlist, datalist, type="innovation.biomass")
plot.sim(simlist, sobj, tlist, parlist, datalist, type="biomass.over.time")
plot.sim(simlist, sobj, tlist, parlist, datalist, type="concentration.over.time")
plot.sim(simlist, sobj, tlist, parlist, datalist, type="observed.concentration.over.time")
plot(tvec,apply(simlist$cmat,1,max), xlab="Week", ylab="Maxmimum observed concentration",
     main="Maximum concentration"); lines(tvec, apply(simlist$cmat,1,max))
abline(v=tvec[simlist$ievent$istart],col="green")
abline(v=tvec[simlist$ievent$iend],col="red")
```

```{r}
plot.sim(simlist, sobj, tlist, parlist, datalist, type="map.concentration",it1=1,it2=5)
```
...
make.movie <- TRUE
make.movie <- FALSE
#plot.sim(simlist, sobj, tlist, parlist, datalist, type="map.concentration",it1=1,it2=tlist$nt,
