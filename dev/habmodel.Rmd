 ...
 * At time step $t$ the epoch indicator $E_t|E_{t-1}$ is drawn
   ...
   (Q: do we need spatial autocorrelation as well?  how can we include covariates?)

 * Growth rate in polygon $i$ at time $t$
   \[
      Q_{it} = \exp\left( -\gamma(1-E_t) 
                          + E_t(\alpha + {\bf x}_{it}^T{\beta}_2 + \lambda_2\log R_{it}) 
                   \right)
   \]
   If $E_t=1$ we have growth ($\alpha>0$, $\lambda_2>0$) and if $E_t=0$ we have
   decay ($\gamma>0$).
 * Carrying parameter of polygon $i$ at time $t$
   \[
      K_{it} = V_i \rho_c R_i^{\lambda_c} = V_i e^{\alpha_c + \lambda_c \log R_{it}}
   \]
   Note that a polygon with zero residency cannot retain mass in this model.
   
   The carrying capacity of the polygon is $M_{c,it}=K_{it}(Q_{it}-1)/Q_{it}$

 * $\pi_{it}$ bloom initiation probability in polygon $i$ at time $t$
   \[
      \logit(\pi_{it}) = \frac{A_i}{a_0}\left[\mu_0 + {\bf x}_{it}^T{\beta}_0 
                         + \lambda_0 \log(R_{it})\right]
   \]
 ...
 * Time evolution of algal mass $M_{it}$ for $t>0$
   \[
      M_{it} = e^{\eta_{it}}\left[
               B_{it} + e^{-\gamma (1-E_t) + E_t(\alpha + {\bf x}_{it}^T{\beta}_2 + \lambda_2 \logit R_{it})}
                        \sum_{j=1}^n P_{ijt} 
   ...
   $\alpha>0$ is the basic (covariate independent) growth rate of blooms in a favourable epoch $E_t=1$,
   $\gamma>0$ is the decay rate of blooms in an unfavourable epoch $E_t=0$.
...
* $(a_0,\lambda_0,\beta_0)$ parameters of bloom initiation probability
...
a0 <- 1
#mu0 <- -6.0; lambda0 <- 3.03
# mu0 <- -5.8; lambda0 <- 3.1
mu0 <- -5.2; lambda0 <- 2.7
pavec <- expit((areavec/a0)*(mu0 + lambda0*logit(resvec)))
...
breaks <- 0.5+0:max(apply(imat,2,sum))
...
for(t in 2:nt) {
  dvec <- exp(evec[t]*lambda1*logit(resvec))
  mvec <- pmat%*%diag(dvec)%*%mmat[t-1,]
  mvec <- btmat[t,] + exp(-gamma*(1-evec[t]) + evec[t]*(alpha+lambda2*logit(resvec)))*mvec
...
```{r}
colnames(mmat) <- names(pavec)
plot(mmat[,"200"])
plot(mmat[,"32"])
plot(mmat[,"32"],mmat[,"357"])
```
